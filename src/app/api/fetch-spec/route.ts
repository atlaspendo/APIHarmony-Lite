...
export async function POST(request: NextRequest) {
  ...
  let specUrl = ''; // Initialize specUrl

  try {
    const body = await request.json();
    specUrl = body.url; // Assign specUrl here
    ...
    rawSpecTextForOutput = YAML.dump(validatedSpec); 
    
    return NextResponse.json({ specObject: validatedSpec, rawSpecText: rawSpecTextForOutput });

  } catch (error: any) {
    console.error(`Error in fetch-spec proxy for URL: ${specUrl || 'Unknown URL provided in request'}. Details:`, error.message);
